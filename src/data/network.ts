...
import companyHubData from './companyHubs.json';
...
type CompanyHubJson = Omit<CompanyHub, 'logoPath' | 'visitorFact'>;

export const COMPANY_HUBS: CompanyHub[] = (companyHubData as CompanyHubJson[]).map((hub) => ({
  ...hub,
  logoPath: assetPath(hub.logo),
  visitorFact: hub.description,
}));
